...
use App\Filament\Resources\SetoranStbResource\Pages;
use App\Filament\Resources\SetoranStbResource\RelationManagers;
use App\Models\SetoranStb;
use Filament\Forms;
...
class SetoranStbResource extends Resource
{
    protected static ?string $model = SetoranStb::class;
...
protected static ?string $navigationLabel = 'Setoran';
    public static function form(Form $form): Form
    {
        ...
        ->schema([
            ...
            ->schema([
                ...
                ->label('nama')
                ->options(StokStb::query()->pluck('cabang1', 'id'))
                ->reactive()
                ->afterStateUpdated(fn ($state, callable $set) => $set('total_tagihan', StokStb::find($state)?->total_tagihan ?? 0)),
            // Select::make('cabang')
    ...
    {
        return [
            'index' => Pages\ListSetoranStbs::route('/'),
            'create' => Pages\CreateSetoranStb::route('/create'),
            'edit' => Pages\EditSetoranStb::route('/{record}/edit'),
        ];
    }    
}
